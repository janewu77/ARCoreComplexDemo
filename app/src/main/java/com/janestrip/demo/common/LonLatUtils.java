package com.janestrip.demo.common;
...
//    }
     ...
     * @param lon1     A的经度
     * @param lat1     A的纬度
     * @param distance AB距离（单位：KM）
     * @param azimuth  AB方位角
     * @return B的经纬度
     */
    public static double[] ConvertDistanceToLogLat(double lon1, double lat1,
                                                   double distance, double azimuth) {
        azimuth = rad(azimuth);
        // 将距离转换成经度的计算公式
        double lon = lon1 + (distance * Math.sin(azimuth))
...
//        return lon + "," + lat;
    ...
    public static List<double[]> GetCiclePoints(double lon1, double lat1,
                                                double distance) {
        List<double[]> list = new ArrayList<>();
        for (int i = 0; i < 360; i += 30) {
            list.add(ConvertDistanceToLogLat(lon1, lat1, distance, i));
        }
        return list;
    }
    ...
    public static void main(String[] args) {
        ...
        double[] location=ConvertDistanceToLogLat(lon1, lat1, distance, azimuth);
        ...
        List<double[]> list=GetCiclePoints(lon1, lat1, distance);
        for (double[] item:list) {
            System.out.println("" +item[0]+", "+item[1]);
        }


        double lon = 121.42842;
        double lat = 31.21852;

        for (int i = 0; i < 360; i += 90) {
            double[] item = ConvertDistanceToLogLat(lon, lat, 1, i);
            System.out.println("====:" +item[0]+", "+item[1]);
